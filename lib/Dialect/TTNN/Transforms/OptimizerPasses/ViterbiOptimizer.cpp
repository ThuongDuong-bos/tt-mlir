...
private:
  static RankedTensorType getTensorTypeWithLayout(RankedTensorType inputType,
    ...
    if (!mlir::isa<mlir::quant::QuantizedType>(elementType)) {
      ...
      auto quantizedType = mlir::cast<mlir::quant::QuantizedType>(elementType);
  ...
  static Value materializeToLayout(Value input, TTNNLayoutAttr outputLayout,
    ...
    Location loc =
        ttmlir::utils::appendLocationSuffix(insertBefore->getLoc(), locSuffix);
  ...
  static void processTransitionEdges(
    ...
    for (const analysis::TransitionEdge &transitionEdge : transitionEdges) {
      ...
      if (!consumerOp || consumerOp->getParentOfType<func::FuncOp>() != func ||
          transitionEdge.consumerOperandIndex >= consumerOp->getNumOperands()) {
        continue;
      }

      Value input = consumerOp->getOperand(transitionEdge.consumerOperandIndex);
      auto inputType = mlir::dyn_cast<RankedTensorType>(input.getType());
  ...
  static void processSpillRequests(llvm::ArrayRef<SpillRequest> spillRequests,
                                   func::FuncOp func) {
    auto isUseAfterTrigger = [](Operation *useOp, Operation *triggerOp) {
      return useOp && triggerOp && useOp->getBlock() == triggerOp->getBlock() &&
    ...
    for (const SpillRequest &spillRequest : spillRequests) {
      ...
      for (OpOperand *use : laterUses) {
        ...
        Value reloadValue = materializeToLayout(spillValue, valueLayout, userOp,
                                                "_reload_from_dram");
        if (!reloadValue) {
          continue;
        }
  ...
  static void processReallocationValues(
    ...
    for (const auto &[_, values] : memReallocateValuesMap) {
      for (Value value : values) {
        ...
        Location loc = ttmlir::utils::appendLocationSuffix(producerOp->getLoc(),
                                                           "_reallocate");
...
void ViterbiOptimizer::runOnOperation() {
  ...
  ttcore::GridAttr deviceGrid = ttcore::lookupDevice(moduleOp).getWorkerGrid();
  ...
  if (memoryLayoutAnalysisEnabled) {
    ...
    opCandidatesBuilder.buildPrunedCandidates(tensorTypePossibleLayouts,
                                              opSchedule, legalConfigs);
    ...
    ViterbiPolicy viterbiPolicy(
        opCandidateResult, prunedGraphInfo.prunedSchedule, prunedGraphInfo);
  ...
  if (memoryLayoutAnalysisEnabled && reallocationAnalysisEnabled) {
    ...
    llvm::DenseMap<Operation *, OpConfig> reallocationOpConfigMap = opConfigMap;
    ...
    reallocationAnalysis.init(ReallocationAnalysisInput(
        std::move(reallocationOpConfigMap), std::move(reallocationSchedule),
        utils::getUsableL1PerCore(moduleOp), reallocationOffsetCapacity));
    ...
    processReallocationValues(reallocationResult.memReallocateValuesMap,
                              builder);
  }
